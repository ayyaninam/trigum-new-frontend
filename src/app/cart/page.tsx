...
const page = () => {
  ...
  return (
    <div className='rounded-lg border-2 border-orange-400 p-4 my-16 container mx-auto'>
      ...
      <div className='divide-y divide-gray-100 text-black h-96 max-h-96 overflow-auto'>
        ...
        {!loading ? cartProducts && cartProducts?.results && cartProducts?.results?.length > 0 ? cartProducts.results?.map((product) => (
          <CartTable
            key={product?.id}

...
import {
  getCompProductsAllIds,
  getPairProductsAllIds,
} from "@/processor/custom";
...
const ComparisonPage: React.FC = () => {
  ...
  const fetchProduct = async () => {
    console.log(
      `${
        process.env.API_URL
      }/api/tyreadderapp/products/?ids=${compProductsIds.join(",")}`
    );
  ...
  const handleImageClick = (
    ...
    const clickedImageUrl = products.results[productIndex].image_urls.splice(
      imageIndex,
      1
    )[0];
    ...
    <>
      ...
      {compProductsIds && compProductsIds.length > 0 && (
        <div className="container mx-auto">
          <div className="font-serif text-center text-2xl font-bold">
            {t("ConvertLine")}
          </div>
          ...
          <div className="bg-gradient-to-r from-transparent via-orange-400 to-transparent py-8 font-bold text-center  flex flex-col mb-8">
            <span className="text-black text-sm">{t("Youarecomparing")}</span>
            <span className="text-white text-5xl">
              {compProductsIds?.length} Products
            </span>
            <span className="text-black text-sm">{t("rightNow")}...</span>
          </div>
        </div>
      )}
